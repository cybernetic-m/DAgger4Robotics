...
if render == False and video_saving==False:
    env = gym.make("FrankaKitchen-v1",tasks_to_complete=['microwave'],robot_noise_ratio=robot_noise) # Cration of the environment only for gathering data
else: #at least one True of render or video_saving
    env = gym.make("FrankaKitchen-v1",tasks_to_complete=['microwave'], render_mode="rgb_array", robot_noise_ratio=robot_noise) # Creation of the Reacher-v5 environment with rgb_array mode
